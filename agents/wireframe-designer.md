...
- **Screen boxes (nodes):**
  - **Compact mode (default):** Rectangle ~180×120px — chỉ hiển thị screen name + purpose + component count badge
  - **Visual mode:** Rectangle ~200×280px — hiển thị thêm **mini wireframe layout** bên trong box (SVG với các component boxes đơn giản, grayscale, tỷ lệ ~0.25x)
  ...
  - Visual preview area: ~160×180px (bên dưới text), SVG inline, các boxes đại diện cho components
  ...
  - **Toggle button** trên header bar để switch giữa Compact/Visual mode cho toàn bộ flowchart
...
#### 4. Visual Preview (on Flowchart Nodes)

**Mini wireframe layout hiển thị TRỰC TIẾP trên flowchart nodes** — không chỉ trong detail panel:

- **2 chế độ view:**
  - **Compact (default):** Box 180×120px — chỉ hiển thị screen name + purpose + component count badge
  - **Visual:** Box 200×280px — hiển thị thêm mini wireframe preview bên trong box (~160×180px SVG)

- **Mini preview trong node:**
  - SVG inline với các boxes đại diện cho components
  - Style: wireframe grayscale (border #4b5563, fill white/#f3f4f6)
  - Tỷ lệ: ~0.25x từ mobile viewport (390×844 → fit trong ~160×180px area)
  - Labels: component ID hoặc type (font-size nhỏ ~8px, chỉ hiển thị nếu đủ chỗ)
  - Chỉ hiển thị relative positions, không cần pixel-perfect

- **Toggle:** Button "Compact/Visual" trên header bar để switch toàn bộ flowchart giữa 2 mode

- **Interactivity:**
  - Hover node → highlight border (như cũ)
  - Click node → mở detail panel (như cũ)
  - Click toggle → switch mode cho tất cả nodes cùng lúc

```javascript
// Visual preview data trong WIREFRAME_DATA.screens[].visualPreview
visualPreview: {
  width: 390,   // mobile viewport width
  height: 844,  // mobile viewport height
  boxes: [
    { id: 'navbar-top', x: 0, y: 0, w: 390, h: 60 },
    { id: 'coin-display', x: 20, y: 80, w: 100, h: 40 },
    { id: 'primary-cta', x: 95, y: 400, w: 200, h: 60 }
    // ... auto-generated từ component positions
  ]
}
```

**Wireframe đơn giản của màn hình này** — render dạng SVG với các boxes đại diện cho components:
- Canvas nhỏ (~300×400px hoặc tỷ lệ theo màn hình thực)
- Mỗi component là một box với border, label là component ID (hoặc type nếu ID dài)
- Box position dựa trên `position` field (top/center/bottom/left/right/grid)
- Màu: grayscale, border #4b5563, fill white/light gray
- Scrollable nếu màn hình dài
- Hover vào box trong preview → highlight row tương ứng trong component table bên dưới
- Click box trong preview → scroll đến và highlight component đó trong table

```javascript
// Ví dụ cấu trúc preview data
visualPreview: {
  width: 390,   // mobile viewport width
  height: 844,  // mobile viewport height
  scale: 0.35,  // scale down to fit panel
  boxes: [
    { id: 'navbar-top', x: 0, y: 0, w: 390, h: 60, type: 'navbar' },
    { id: 'coin-display', x: 20, y: 80, w: 100, h: 40, type: 'label' },
    // ... tự động generate từ component positions
  ]
}
```
...
- Action buttons: 
  - "Compact/Visual toggle" — switch giữa box nhỏ (180×120) và box lớn có mini wireframe preview (200×280)
  - "Expand all panels" — dump full spec vào 1 scroll
  - "Print view" — CSS print-friendly
  ...
  screens: [
    {
      ...
      visualPreview: {
        ...
        scale: 0.35,
        boxes: [
          { id: 'navbar-top', x: 0, y: 0, w: 390, h: 60, label: 'navbar-top' },
          { id: 'coin-display', x: 20, y: 80, w: 100, h: 40, label: 'coin-display' },
          { id: 'primary-cta', x: 95, y: 400, w: 200, h: 60, label: 'Play' }
        ]
      },
      openQuestions: []
      openQuestions: []
...
- [ ] Visual Preview trong flowchart nodes hoạt động (2 mode: Compact/Visual)
- [ ] Compact mode: Box 180×120px hiển thị text info
- [ ] Visual mode: Box 200×280px có mini wireframe preview (~160×180px SVG)
- [ ] Mini preview hiển thị đúng layout component (boxes + labels, grayscale)
- [ ] Toggle button trên header bar switch được giữa 2 mode
- [ ] Hover box trong preview highlight row trong table (và ngược lại)
...
- KHÔNG ĐƯỢC exceed 3000 dòng trong file HTML duy nhất (bao gồm Visual Preview code)
