...
| Add prototype serving | `src/tools/prototype.ts` | Uses `Bun.serve()` for local HTTP server |
...
- Depends on root package via `"knowledge-layer": "file:.."`.
...
- Prototype tool manages a `Bun.Server` lifecycle (start/stop) for local preview.
